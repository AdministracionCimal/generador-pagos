...
2. el match contra `composicionSaldoProveedor` (si no coincide, el proveedor desaparecía como "sin saldo"). Se cruza contra `DOCUMENTO` / `COMPROBANTE` / `IDENTIFICACIONEXTERNA` y alcanza con una — ver la nota de 2026-08-24 más abajo
...
- misma familia, en la numeración de cheques: `numero_cheque_desfasado()` devuelve `None` tanto si el número coincide como si **no se pudo leer el talonario**, y el segundo caso hacía desaparecer en silencio la protección contra cheques con número repetido. `erp_informo_ultimo_cheque()` separa los dos y ahora se pregunta antes de emitir
- el control de saldos es **fail-open**: si `composicionSaldoProveedor` falla, el proveedor queda con `pendientes = None` y se paga sin verificar. Antes eso era mudo (la barra igual decía «Verificando saldo n/m»); ahora `componer_avisos()` lo nombra en el diálogo previo al envío. No bloquea: un timeout del ERP no debería frenar la tanda
- **`AplicacionOrigen` del POST también resuelve por `IdentificacionExterna`** (verificado contra el ERP el 2026-08-24, posteando en la empresa `Pueba`): con `FC - 22219` la OP se crea con **200 created** pero **no queda aplicada** — Finnegans lo trata igual que a un documento inexistente. Método de verificación: aplicar contra una factura de otra empresa devuelve 500 «No se pueden aplicar transacciones de distintas empresas» **sólo si resolvió el identificador**; un 200 significa que no lo encontró. `FC - 20402` (normal) → 500; `FC - 22219` → 200; `20313144411-A-0002-00000196` → 500. Por eso `_construir_ops` completa `ItemFactura.aplicacion_origen` con la `IDENTIFICACIONEXTERNA` que devolvió `composicionSaldoProveedor` y el mapper manda esa. Para lo que carga Finnegans el valor no cambia (ahí `IDENTIFICACIONEXTERNA == DOCUMENTO`)
- `composicionSaldoProveedor` consulta por CUIT y **mezcla todo el grupo** si no se filtra: para un proveedor volvían 9 filas (2 de CIMALCO, 1 de RTC, 6 de la empresa de pruebas). Se filtra con `PARAMWEBREPORT_Empresa` — **con `E` mayúscula: en minúscula se ignora en silencio** — y el código de negocio limpio (`EMPRE01`; con `EMPRESA_EMPRE01` devuelve cero filas). Como el filtro es del servidor y puede no tomar, `filtro_de_empresa_aplicado()` verifica que la respuesta traiga una sola empresa; si no, se trata como «no verificado» y el aviso de fail-open lo dice
- **`GET /facturaCompra/{clave}` resuelve por `IdentificacionExterna`, no por el documento interno** (detectado 2026-08-24): con `FC - 22219` devuelve **404** para las facturas que carga el otro sistema. El fallo era mudo (`except Exception: pass`) y sin ratio gravado/total se asume 100% gravado, así que **la retención sale de más y al proveedor se le paga de menos** — medido: $63.000 en un solo pago de 5 facturas. `get_factura_compra()` reintenta con `documento.id_externa(cuit, comprobante)` = `<CUIT>-<comprobante>`, y si aun así no resuelve ahora avisa
- **`IDENTIFICACIONEXTERNA` dejó de servir como clave de cruce** (detectado 2026-08-24): venía igual a `DOCUMENTO` y el control de saldos se hacía sólo contra ella. Desde que se cargan facturas desde otro sistema, ese campo trae `<CUIT>-<LETRA>-<PTOVTA>-<NÚMERO>` (`20313144411-A-0002-00000196`), que no matchea con el Excel: la factura se daba por «sin saldo» y **se omitía del pago sin avisar** (5 facturas, $18,3 M, ya en producción). `documento.claves_pendientes()` ahora indexa `DOCUMENTO` + `COMPROBANTE` + `IDENTIFICACIONEXTERNA` y alcanza con que pegue una. El Excel exporta `Documento` y `Comprobante`, que se corresponden con los dos primeros — `DOCUMENTO` es el confiable (155/155 filas poblado). El CUIT de esa identificación se canoniza a 11 dígitos (`20-31314441-1-A-…` ≡ `20313144411-A-…`): las dos formas son el mismo comprobante y no se le pide al otro sistema que respete una convención que nadie valida
- **el formato de fecha de `ApiSituacionCheques` cambió sin aviso** (detectado 2026-08-24): hasta julio 2026 devolvía `dd-mm-yyyy` y ahora manda ISO `yyyy-mm-dd`. `cartera._fecha()` acepta los dos. Si se rompe de nuevo ya no falla en silencio: la fecha queda en `None` y `pago_combinado._a_endosado()` levanta `RepartoError` («vino sin fecha de…»), así que el proveedor va a **carga manual** con el motivo a la vista. Antes ese `None` viajaba como `FechaDocumentoFisico: ""` y Finnegans lo aceptaba igual — el endoso quedaba cargado sin fechas y nadie se enteraba
