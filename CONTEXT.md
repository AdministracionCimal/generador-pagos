...
- **el formato de fecha de `ApiSituacionCheques` cambió sin aviso** (detectado 2026-08-24): hasta julio 2026 devolvía `dd-mm-yyyy` y ahora manda ISO `yyyy-mm-dd`. `cartera._fecha()` acepta los dos. Falla en silencio si se rompe de nuevo: la fecha queda en `None`, el POST del endoso viaja con `FechaDocumentoFisico: ""` y Finnegans lo acepta igual
